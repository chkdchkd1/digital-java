...
public class Ex1 {
	...
	public static void main(String[] args) {
		...
		*/
		 ...
		 *   - public : 나 + 같은 패키지 + 자식 클래스(상속받은 클래스) + 다른 패키지 
		 *   - protectd : 나 + 같은 패키지 + 자식클래스(상속받은 클래스) 
		 *   - default : 나 + 같은 패키지  (접근제한자를 아예 안쓰는거 ) 
		 *   - private : 나  */
		 ...
		 *   throw
		 *   메서드에 throws 를 통해 예외를 꼭 적어줘야하는 경우  = RuntimeException이 아닌경우 
		 *   
		 *  - 대표적인 예외 상황
		 *   1. 0으로 나누었을 때 
		 *   2. 배열의 번지가 잘못된 번지인 경우 : 음수나 배열의 크기보다 같거나 큰 경우
		 *   3. 객체가 생성되지 않아서 NULL값인데 객체의 멤버 메서드에 접근하려 할 때 
		 *   4. 클래스의 형변환이 되지 않는데 형변환 하려 할 
		 *   
		 *  - 예외 발생시 RuntimeException인 경우 예외처리를 하지 않아도 문법적으로 문제되지 않음 (밑줄이 뜨지 x)
		 *  - RuntimeException이 아닌 경우는 꼭 예외 처리를 해줘야한다. 안하면 에러 발생
		 * */
		 ...
		 *- 클래스에서 일반적으로 멤버 변수는 private . 멤버 메서드는 public으로 접근제한자를 둔다
		 *- 메서드 오버라이딩 : 부모 클래스에 있는 메서드를 재정의하는 것 (1. 부모 클래스의 메소드의 선언부가 완전히 동일)
			간단히 정의하면 오버로딩(overloading)은 새로운 메소드를 정의하는 것입니다.
			하지만 오버라이딩(overriding)은 상속받은 기존의 메소드를 재정의하는 것입니다.*/
		
		/*- 인터페이스 : 추상메서드와 상수만 있는것 
		 *- 추상 메서드 : 메서드 구현부가 없이 선언부만 있는 메서드
		 *		--> 접근 제한자 리턴타입 메서드명(매개변수){
		 *					구현;}
		 *		--> 접근제한자 리턴타입 메서드명(매개변수); <추상메서드>
		 *          - (어떠한 기능이 있는지 일목요연하게 볼 수 있다) 
		 *          - 협업할 때 인터페이스를 작성해놓고 기능을 구현하면 개발자들 사에에서 메서드에 대한 형태를 일치 시킬 수 있다. 
		 *          ----> 표준화 하기 쉽다 
		 * */
		
		/*컬렉션 프레임워크 : 자료를 효율적으로 관리하기 위해 정형화 해 놓은것
		 * - 컬렉션 - List. set : value를 관리
		 * - Map - key, Value를 관리 
		 * - List 인터페이스, ArrayList는 List 인터페이스를 구현한 클래스 
		 * 		- 순서 보장, 중복허용
		 * - Set 인터페이스, HashSet는 set을 구현한 클래스 
		 *  	- 순서 보장 X, 중복 허용X
		 *  - Map은 인터페이스, HashMap은 Map을 구현 클래스
		 *  - key, value로 이루어짐
		 *  - key는 중복 불가 value 중복 가능
		 *  - 순서보장 X (중복 안되면 대게 순서 보장 x)
		 * */
		 ...
		 * 데어터가 중복되지 않는다  - set
		 * 데이터의 순서가 중요하다 - list
		 * 데이터가 아이디,패스워드 처럼 세트로 관리해야한다 Map*/
