...
public class LottoEx3 {
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> lotto  = new ArrayList<Integer>();  //당첨번호 
		ArrayList<Integer> auto; //두가지 방법으로 해보기위해서 new를 쓰지 X 것일뿐
		int min = 1, max = 10;		
		int bonus = 0;		
		try {
	...
	public static void createRandomSet(int min, int max, ArrayList<Integer> lotto, int cnt) throws Exception {
	...
	public static ArrayList<Integer> createRandomSet(int min, int max, int cnt) throws Exception{
		if( cnt > max-min+1)
			throw new Exception("예외 : 랜덤한 수의 범위보다 배열의 크기가 큽니다 ");
		ArrayList<Integer> list = new ArrayList<Integer>();
			while (list.size() < cnt) {
				int r = random(min,max);
				if (!list.contains(r)) //but list는 중복을 거르지 않기 때문에 필요하다 
	...
	public static void printSet(ArrayList<Integer> lotto) {
	 ...
	 * 메소드명 : rank
	 * */
	public static int rank(ArrayList<Integer> lotto, int bonus, ArrayList<Integer> auto) {
		int sameCnt = 0;
